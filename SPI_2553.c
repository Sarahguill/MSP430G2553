...
void Send_char_SPI(unsigned char SendSPI);
...
/*#define delay_cycles 250		/* Sleep time */*/
...
UINT_32 car = 0x30;       /* 0 */
UINT_32 nb_car = 0;
BOOL intcmd = FALSE;   /* call interpreteur() */
 ...
 * Sorties:  -
 */
void envoi_msg_UART(UCHAR_32 *msg)
{
    UINT_32 i = 0;
    for(i=0 ; (UINT_32)msg[i] != 0x00 ; i++)
    {
        while(!(IFG2 and UCA0TXIFG)){};
	  /*attente de fin du dernier envoi (UCA0TXIFG à 1 quand UCA0TXBUF vide)*/
        (UCHAR_32)UCA0TXBUF=msg[i];
    }
}
